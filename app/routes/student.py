...
from app.services.geo_service import bounding_box, find_within_radius
...
def teacher_search():
    ...
    lat_min, lat_max, lon_min, lon_max = bounding_box(profile.latitude, profile.longitude, radius_km)
    ...
    if subject:
        ...
        candidates = [
            ...
            if any(
                kw in t.subjects.lower()
                for kw in keywords
            )
        ]

    results = find_within_radius(candidates, profile.latitude, profile.longitude, radius_km)
